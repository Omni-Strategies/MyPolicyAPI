...
async def get_requests_with_customer_details_excluding_quoted(
...
async def get_insurance_request_with_customer(
...
@router.get("/me")
async def get_my_insurance_requests(
    db: Session = Depends(get_db),
    current_user=Depends(customer_required),
):
    logger.info(f"Fetching insurance requests for customer {current_user['id']}")
    return requests_repo.get_requests_by_customer(db, uuid.UUID(str(current_user["id"])))
