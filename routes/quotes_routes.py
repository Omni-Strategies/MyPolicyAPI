...
from security.dependencies import company_admin_required, customer_required
...
def get_quotes_with_details_by_customer(
    customer_id: uuid.UUID,
    db: Session = Depends(get_db),
    current_user=Depends(customer_required)
):
    try:
        quotes = repo_get_quotes_with_details_by_customer(db, customer_id)
        if not quotes:
            raise HTTPException(status_code=404, detail="Quotes not found")
        return quotes
    except Exception as e:
        logger.error(f"Error fetching quotes for {customer_id}: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail={str(e)}) 
